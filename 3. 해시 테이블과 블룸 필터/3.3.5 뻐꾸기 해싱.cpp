...
class hash_map { // 두개의 해시 테이블을 사용할 것이고 각 테이블의 크기를 멤버로 저장했습니다.
	std::vector<int> data1;
	std::vector<int> data2;
	int size;

	int hash1(int key) const { // 해시 함수는 필요에따라 적절하게 구성하면됩니다
		return key % size;
	}
...
public:
	hash_map(int n) : size(n) { // 해시테이블은 모두 -1로 초기화했으며 이는 비어 있음을 의미합니다.
