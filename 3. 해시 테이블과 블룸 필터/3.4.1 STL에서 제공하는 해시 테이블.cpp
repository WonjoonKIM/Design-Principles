...
void print(const std::unordered_set<int>& contatiner) { // 컨테이너에 들어 있는 모든 원소를 출력하는 print()함수
	for (const auto& element : contatiner)
		std::cout << element << " ";
	std::cout << std::endl;
}

void print(const std::unordered_map<int, int>& contatiner) {
	for (const auto& element : contatiner)
		std::cout << element.first << " : " << element.second << ", ";
	std::cout << std::endl;
}

void find(const std::unordered_set<int>& container, const int element) { // 원소 검색 결과를 출력하기 위한 find()함수
