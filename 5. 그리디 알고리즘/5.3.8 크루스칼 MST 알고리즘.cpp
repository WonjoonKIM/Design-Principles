...
public:
	...
	//디스조인트-셋 자료 구조에서 두 트리를 합치는 연산을 구현합니다. 
...
//크루스칼 최소 신장 트리 알고리즘을 구현합니다.
template <typename T>
Graph<T> minimum_spaning_tree(const Graph<T>& G) {
	//에지 가중치를 이용한 최소 힙 구성
...
int main() {
	...
	for (auto& i : edge_map)
		for (auto& j : i.second)
			G.add_edges(Edge<T>{i.first, j.first, j.second});

	cout << "[입력 그래프]" << endl;
	cout << G << endl;

	Graph<T> MST = minimum_spaning_tree(G);
	cout << "[최소 신장 트리]" << endl;
	cout << MST;
