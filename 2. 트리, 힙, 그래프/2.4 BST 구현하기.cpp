...
private:
	node* find_impl(node* current, int value) {
		...
		if (value < current->data) { // value가 현재 노드보다 작은 경우
			std::cout << current->data << "에서 왼쪽으로 이동 : ";
			return find_impl(current->left, value);
		}
		std::cout << current->data << "에서 오른쪽으로 이동 : "; // value가 현재 노드보다 큰 경우
...
private:
	void insert_impl(node* current, int value) { // 만약 값을 삽입해야 할 쪽이 비어 있으면 새로운 노드를 추가하고, 이미 다른 노드가 있다면 삽입 함수를 재귀적으로 호출합니다.
...
private:
	node* delete_impl(node* start, int value) {
		...
		if (value < start->data)
			...
			if (!start->right) {
				auto tmp = start->left; // 오른쪽 자식 노드만 없는 경우
				delete start;
				return tmp;
			}
			//자식 노드가 둘 다 있는 경우
